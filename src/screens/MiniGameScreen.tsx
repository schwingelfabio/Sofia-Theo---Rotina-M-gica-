...
export const MiniGameScreen: React.FC<MiniGameScreenProps> = ({ taskId, onComplete, onCancel }) => {
  const { completeRoutine, addHearts, language } = useGame();
  ...
  return (
    <div className="absolute inset-0 z-40 bg-white/90 dark:bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center">
      ...
      <AnimatePresence mode="wait">
        {progress < totalSteps ? (
          <motion.div
            ...
            className="flex flex-col items-center max-w-md w-full bg-[#F0F9F9] dark:bg-slate-800 rounded-[40px] shadow-2xl p-8 border-4 border-white dark:border-slate-700"
          >
            ...
            <p className="text-[#2D3748]/70 dark:text-slate-300 font-bold pb-8">{isPt ? 'Vamos fazer isso juntos!' : "Let's do this together!"}</p>
            
            <div className="bg-white dark:bg-slate-900 w-full h-64 rounded-[32px] flex items-center justify-center relative overflow-hidden shadow-inner border-2 border-[#008080]/10 dark:border-slate-700">
               <motion.button
                 whileHover={{ scale: 1.1 }}
                 whileTap={{ scale: 0.9 }}
                 onClick={handleStep}
                 className="w-24 h-24 bg-[#FF69B4] rounded-[24px] text-white flex items-center justify-center shadow-lg hover:bg-[#ff52a3]"
               >
